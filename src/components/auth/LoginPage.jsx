...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && userRole && !requiresPasswordChange) {
      navigate(userRole === 'admin' ? '/admin' : '/ra');
    }
  ...
  const canSubmitChange = allReqsMet && passwordsMatch && !changeLoading;
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (result.requiresPasswordChange) return; // UI switches automatically
      navigate(result.role === 'admin' ? '/admin' : '/ra');
    } catch (err) {
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      ...
      if (err.message?.includes('Password does not conform')) {
        setChangeError('Password does not meet the requirements. Please try again.');
      ...
      <div className="lp-bg">
        <div className="lp-card">
          ...
          <p className="lp-subheading">
            Your identity has been verified. Please create a new password to activate your account.
          </p>
          ...
          <form onSubmit={handlePasswordChange} className="lp-form" noValidate>
                ...
                {passwordRequirements.map((req, i) => {
                  ...
                  return (
                    <li key={i} className={`lp-req ${met ? 'met' : ''}`}>
                      <span className="lp-req-icon">{met ? <CheckIcon /> : <span className="lp-req-dot" />}</span>
                      {req.label}
                    </li>
                  );
                })}
            ...
            <button
              type="submit"
              className={`lp-btn ${canSubmitChange ? 'lp-btn-active' : 'lp-btn-disabled'}`}
